...
import { AlertCircle, Clock, CheckCircle2, Flag, LogOut } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      ...
      if (document.visibilityState === "hidden") {
        ...
        if (newCheatCount > (roomData?.cheat_limit || 3)) {
          setIsLocked(true);
          await update(attemptRef, { status: "LOCKED", cheatCount: newCheatCount });
        } else {
          await update(attemptRef, { cheatCount: newCheatCount });
          alert(`Peringatan Anti-Cheat! Dilarang meninggalkan halaman ujian. Jumlah pelanggaran: ${newCheatCount}`);
        }
      }
    };
    ...
    <div className="h-screen h-[100dvh] bg-slate-50 dark:bg-slate-900 flex flex-col overflow-hidden">
      ...
      <header className="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 border-b shadow-sm h-14 px-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`font-mono font-bold ${timeLeft < 300 ? "text-red-500 animate-pulse" : "text-slate-700 dark:text-slate-200"}`}>
            {formatTime(timeLeft)}
          </span>
        </div>
        <div>
          <span className="text-sm font-medium text-slate-500">{siswa?.name}</span>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
          ...
          {currentQuestion && (
            <Card className="rounded-xl border shadow-sm">
              ...
              <CardContent className="p-4 pt-0 space-y-2.5">
                {Object.keys(currentQuestion.choices || {}).map((choiceId) => {
        ...
        <div className="w-full md:w-72 bg-white dark:bg-slate-800 border-l p-4 flex flex-col space-y-4">
