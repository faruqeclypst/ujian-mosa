    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-card p-5 rounded-2xl border border-slate-200/60 dark:border-slate-800/40 shadow-sm backdrop-blur-sm">
        ...
        <div className="flex flex-wrap gap-2">
          <ImportButton onImport={handleImportClasses} isLoading={isImporting} />
          <ExportButton onExport={handleExportClasses} />
          <Button onClick={() => downloadClassImportTemplate()} variant="secondary" size="sm" className="rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 dark:bg-slate-900/30 dark:text-slate-400 dark:hover:bg-slate-900/50 dark:border-slate-800/40 text-slate-600 font-semibold shadow-sm">
            Template
          </Button>
      ...
      {loading ? (
        ...
        <ClassTable classes={classes} onEdit={handleEditClick} onDelete={handleDeleteClick} />
      )}
