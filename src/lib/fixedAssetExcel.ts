...
const rowSchema = z.object({
  "Kode Aset Tetap": z.string().min(2, "Kode Aset Tetap wajib diisi"),
  "Nama Aset Tetap": z.string().min(2, "Nama Aset Tetap wajib diisi"),
...
export function downloadFixedAssetImportTemplate(filename?: string): void;
export function downloadFixedAssetImportTemplate(opts?: { roomNames: string[]; filename?: string }): void;
export function downloadFixedAssetImportTemplate(
  arg: string | { roomNames: string[]; filename?: string } = { roomNames: [] }
) {
  const opts = typeof arg === "string" ? { roomNames: [], filename: arg } : (arg ?? { roomNames: [] });
  const filename = opts.filename ?? "template-import-aset tetap.xlsx";
  ...
  const wsNotes = XLSX.utils.aoa_to_sheet([
    ["Keterangan Import Aset Tetap"],
    [],
    ["Cara pakai:"],
    ["1) Klik tombol Download Template di aplikasi."],
    ["2) Isi data pada sheet 'Aset Tetap' mulai dari baris 2."],
    ["3) Simpan file (format .xlsx) lalu Import Excel di aplikasi."],
    [],
    ["Aturan penting:"],
    ["- Kolom wajib: Kode Aset Tetap, Nama Aset Tetap, Jumlah, Tanggal Perolehan, Sumber, Ruangan, Kondisi."],
    ...
    [],
    ["Catatan:"],
    ["- Foto Aset Tetap tidak bisa diimport via Excel. Silakan upload foto saat edit aset tetap di aplikasi."],
  ]);
...
export async function parseFixedAssetImportExcel(file: File): Promise<ParsedInventoryImportItem[]> {
  ...
  rawRows.forEach((row, idx) => {
    ...
    normalizedRow["Kode Aset Tetap"] = toStringCell(normalizedRow["Kode Aset Tetap"]).trim();
    normalizedRow["Nama Aset Tetap"] = toStringCell(normalizedRow["Nama Aset Tetap"]).trim();
    ...
    parsed.push({
      code: res.data["Kode Aset Tetap"],
      name: res.data["Nama Aset Tetap"],
